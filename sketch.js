...
function setup() {
 ...
 console.log(message)
  ...
  trex.setCollider("circle",0,0,trex.width/2+10);
  trex.debug = false;
...
function draw() {
  ...
  fill("black");
  textSize(15);
  text("Score: "+ score, width-100,height/2-50 );
  ...
  if(gameState === PLAY){
    ...
    if((keyDown("space")||touches.length>0)&& trex.y >= height-75) {
        trex.velocityY = -12;
        jumpSound.play();
      touches = [];
    }
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    ...
    cloud.y = Math.round(random(height-150,height-200));
